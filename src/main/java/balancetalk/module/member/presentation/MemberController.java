...
public class MemberController {
    ...
    public MemberResponseDto updateNickname(@PathVariable("memberId") Long memberId, @Valid @RequestBody NicknameUpdate nicknameUpdate) {
        return memberService.updateNickname(memberId, nicknameUpdate);
    }

    @ResponseStatus(HttpStatus.OK)
    @PutMapping("/{memberId}")
    @Operation(summary = "회원 비밀번호 수정", description = "회원 패스워드를 수정한다.")
    public MemberResponseDto updatePassword(@PathVariable("memberId") Long memberId, @Valid @RequestBody PasswordUpdate passwordUpdate) {
        return memberService.updatePassword(memberId, passwordUpdate);
    }
